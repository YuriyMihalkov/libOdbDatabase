...
* Все классы объектов, которые планируется разместить в SQL-базе данных должны находиться в папке **"entities_database"** для автоматического добавления их в компилятор ODB ORM. Данная папка может находиться в любом месте проекта, так же их может быть несколько и все они будут обработаны при сборке проекта. 
